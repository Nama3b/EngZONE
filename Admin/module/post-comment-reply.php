<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
    <div class="card table-responsive m-b-30">
        <?php 
            $cmt_id = $_GET["id"];
            $sql = "SELECT * FROM tbl_post_comment WHERE cmt_id = '$cmt_id'";
            $result = mysqli_query($conn,$sql) or die("Lỗi truy vấn cmt");
            $row = mysqli_fetch_assoc($result);
         ?>
        <h3 class="card-header">Main comment content: <?php echo $row["cmt_content"] ?></h3>
        <div class="card-body">
            <table class="table table-bordered table-hover" id="bookRv_data">
                ...
                <tbody>
                    <?php 
                        $cmt_id = $_GET["id"];
                        $sqlselectRep = "SELECT * FROM tbl_post_comment_reply INNER JOIN tbl_client ON tbl_post_comment_reply.client_id = tbl_client.client_id WHERE tbl_post_comment_reply.cmt_id = '$cmt_id'";
                        ...
                        <tr class="table-body">
                            ...
                            <td>
                                <?php 
                                    ...
                                    if($status==0){
                                        echo '
                                            <a href="admin.php?module=active-action&reply_id='.$rowRep["reply_id"].'" ><i class="fas fa-lock-open"></i></a>
                                        ';
                                    } else{
                                        echo '
                                            <a href="admin.php?module=block-action&reply_id='.$rowRep["reply_id"].'" ><i class="fas fa-user-lock"></i></a>
                                        ';
                                    }
                                 ?>
