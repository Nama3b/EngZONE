  ...
  <div class="row justify-content-center">
            ...
            <div class="col-12" style="padding: 0;">
              ...
              <div class="card-profile-stats d-flex text-center">
                <div class="col-6">
                  ...
                  <span class="heading"><i class="fa fa-shopping-cart"></i>: <?php echo $rowPost["total_post"] ?>-<button class="btn btn-sm btn-outline-success ml-1"><a href="index.php?module=client-post&client_id=<?php echo $row[0] ?>" style="font-size: 12px; color:green; text-decoration: none;">Detail</a></button></span>
                </div>
                <div class="col-6">
                  <span class="description"><small>Your orders:</small></span><br>
                  <span class="heading"><i class="fab fa-blogger"></i>: <?php echo $rowOrder["total_order"] ?>-<button class="btn btn-sm btn-outline-danger ml-1"><a href="index.php?module=client-order&client_id=<?php echo $row[0] ?>" style="font-size: 12px; color:green; text-decoration: none;">Detail</a></button></span>
                </div>
              </div>
              <div class="card-profile-stats d-flex text-center">
                <div class="col-6">
    ...
    <div class="col-12 col-xs-12 col-sm-12 col-md-12 col-lg-8 col-xl-8 ">
      <div class="card table-responsive">
          <h3 class="card-header">Save lesson's list</h3>
          <div class="card-body">
              <table class="table table-bordered table-hover" id="post_data">
                  ...
                  <tbody>
                      <?php 
                        $sqlselect = "SELECT * FROM tbl_lesson INNER JOIN tbl_lesson_save ON tbl_lesson.lesson_id = tbl_lesson_save.lesson_id WHERE tbl_lesson_save.lesson_status = 1 AND tbl_lesson_save.client_id =".$_SESSION["client_id"];
                        $result = mysqli_query($conn,$sqlselect) or die("Lỗi truy vấn Post");
                        $i = 0;
                       ...
                       ?>
                          <tr class="table-body">
                              ...
                              <td><div class="post-title"><?php echo $row["lesson_name"] ?></div></td>
                              <?php 
                                  $sql = "SELECT * FROM tbl_grade WHERE grade_id =".$row["grade_id"];
                                  $resultGrade = mysqli_query($conn, $sql);
                                  $rowGrade = mysqli_fetch_assoc($resultGrade);
                               ?>
                              <td><?php echo $rowGrade["grade_detail"] ?></td>
                              <td><?php echo $row["teacher"] ?></td>
                              <td><a href="course.php?module=course-lesson-detail&lesson_id=<?php echo $row["lesson_id"] ?>"><i class="fas fa-edit"></i></a></d></td>
                          </tr>
                  <?php } ?>
                  </tbody>
              </table>
